...
renderer.domElement.addEventListener('click', (e) => {
    ...
    if (intersects.length) {
        dynamicBodies.forEach((singleDynamicRigidBody) => {
            singleDynamicRigidBody[0] === intersects[0].object && singleDynamicRigidBody[1].applyImpulse(new RAPIER.Vector3(0, 10, 0), true)
        })
    }
})
